package landvibe.co.kr.destiny01.util;
...
//	private final Base64 b64Decoder = Base64.;
	...
	public String encrypt(String aData) {
		...
		try {
			...
			result = new String(Base64.encode(encryptedData,Base64.NO_PADDING));//this.b64Encoder.encode(encryptedData);
		}
	...
	public String decrypt(String aData) {
		String result = "";
			try {
				...
				byte[] decodedData = new byte[0];//this.b64Decoder.decodeBuffer(aData);

				Log.d("error","decrypt adata : "+ aData );
					decodedData = Base64.decode(aData, Base64.NO_PADDING);
