...
public class PayNormalActivity extends AppCompatActivity {
    ...
    private class MyWebViewClient extends WebViewClient {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            if (url.equals("http://{PAY REDIRECT DOMAIN}/pay/success")) {
                Toast.makeText(PayNormalActivity.this, "결제 완료", Toast.LENGTH_SHORT).show();
                createGeneralChat();
                finish();
            } else if (url.equals("http://{PAY REDIRECT DOMAIN}/pay/fail")) {
                Toast.makeText(PayNormalActivity.this, "결제 실패", Toast.LENGTH_SHORT).show();
                finish();
            }
        }
...
//                        Toast.makeText(PayNormalActivity.this, "intent:// existPackage != null" , Toast.LENGTH_SHORT).show();
                        Log.d("pay", "intent:// existPackage != null");

                    } else {
                        Intent marketIntent = new Intent(Intent.ACTION_VIEW);
                        marketIntent.setData(Uri.parse("market://details?id=" + intent.getPackage()));
//                        Toast.makeText(PayNormalActivity.this, "intent:// else" , Toast.LENGTH_SHORT).show();
                ...
                try {
                    Intent intent = Intent.parseUri(url, Intent.URI_INTENT_SCHEME);
                    if (intent != null) {
//                        Toast.makeText(PayNormalActivity.this, "market" , Toast.LENGTH_SHORT).show();
                        Log.d("pay", "market");
                        startActivity(intent);
                    }
                    return true;
    ...
    public void createGeneralChat() {
        SimpleDateFormat sdfNow = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
        String time = sdfNow.format(new Date(System.currentTimeMillis()));
        Log.d("pay", "user_no : " + user_no);
        Log.d("pay", "company_no : " + company_no);
        //채팅창으로 진입
        ...
        client.post(getString(R.string.URL) + "/history/insert/general", params, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    ...
                    switch ((int) response.getLong("result")) {
                        case -1: {
                            Toast.makeText(PayNormalActivity.this, "역술인이 상담 중입니다.", Toast.LENGTH_SHORT).show();
                            break;
                        }
                        ...
                        // 부재중으로 토스트를 띄운다.
                        case -10: {
                            Toast.makeText(PayNormalActivity.this, "역술인이 부재 중입니다.", Toast.LENGTH_SHORT).show();
                            break;
                        }
                        // 관리자에 의해, User가 BLOCK 상태인 경우
                        case -100: {
                            Toast.makeText(PayNormalActivity.this, "계정이 차단되어 로그아웃 됩니다.\n 고객센터에 문의해주세요.", Toast.LENGTH_LONG).show();
                            checkRegid.logout(PayNormalActivity.this);
                            break;
                        }
                        // 해당 히스토리가 존재할 경우  ->  해당 채팅방으로 입장
                        // 해당 히스토리가 존재하지 않을 경우  ->  채팅방 개설
                        default: {
                            Toast.makeText(PayNormalActivity.this,
                                    "역술인과 상담을 시작합니다.", Toast.LENGTH_LONG)
                                    .show();

                            Log.d("error", "history Insert On success");

                            Intent intent = new Intent(PayNormalActivity.this, ChatActivity.class);
//                        intent.addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY);
                            ...
                            intent.putExtra("chat_start", true);
                    ...
                    e.printStackTrace();
//                    Toast.makeText(PayNormalActivity.this,
//                            "insert Catched" +e, Toast.LENGTH_LONG)
//                            .show();
                }
